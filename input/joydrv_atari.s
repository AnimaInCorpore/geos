...
;
; Atari XL joystick input driver
;
; This driver reads Atari joystick 0 from PIA/GTIA and translates input
; into smooth, accelerated mouse pointer motion.
...
;---------------------------------------------------------------
ReadInput:
	lda PORTA             ; joystick 0 direction bits are active-low
	eor #$ff
	and #%00001111
	sta r11L              ; keep direction nibble while sampling trigger
	lda TRIG0             ; trigger is active-low: 0=pressed
	beq @firePressed
	lda #$00
	beq @mergeRaw
@firePressed:
	lda #%00010000       ; normalize fire button to bit 4
@mergeRaw:
	ora r11L
	pha
